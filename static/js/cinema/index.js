...
const API_KEY = '0100095a9f96d25d2f3a2f69ca848e5b';
...
const getMovies = (url) => {
    const URL = `${URL_PATH}/3/movie/${url}?api_key=${API_KEY}&language=en-MX&page=1`
    return fetch(URL)
        .then(resp => resp.json())
        ...
        html += `
            <div class="carousel-item ${index === 0 ? 'active': null}" style="background-image: url('${titleImage}')">
                <div class="carousel-caption">
                    <h5>${title}</h5>
                    <p class="">${overview}</p>
                    <a href="${`  ../movie.html?id=${id}`}}" class="btn btn-primary">Más información</a>
                </div>
            </div>
        `
    });
